---
title: "Premiers thermiques sérieux de 2025"
...
C'était le `r format(as.Date(date_vol), "%d %B %Y")`. Je suis parti de la région de `r d_post[3]` à `r d_post[2]` et j'ai atterri dans la région de `r d_post[4]` pour un durée de vol de `r d_post[5]`. J'ai volé une fois dans la journée.

C'est un chouette vol après une bonne marche. On avait un tout petit bus. Vraiment trop petit. La marche avec Manu et Jean-Daniel était très sympa très bon décollage avec un joli ventre de face.

Pas mal de monde en l'air qui partait depuis Togon virage. J'ai pas traîné en l'air, j'avais trop mal aux mains, mais ça tenait.
