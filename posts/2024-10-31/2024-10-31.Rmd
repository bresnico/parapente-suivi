---
title: "Premier vol en responsabilité pour Emmanuel !"
...
C'était le `r format(as.Date(date_vol), "%d %B %Y")`. Je suis parti de la région des Cerniers à `r d_post[2]` et j'ai atterri dans la région de `r d_post[4]` pour un durée de vol de `r d_post[5]`. J'ai volé `r rmarkdown::metadata$vols` fois dans la journée. J'ai ressenti `r rmarkdown::metadata$intense` du `r rmarkdown::metadata$emotion`.

C'était le premier vol en responsabilité d'Emmanuel qui réalise bientôt son brevet.

On avait une météo très calme avec un petit vent travers droite depuis Chindonne. C'était assez étonnant parce que le vent de vallée commençait à s'installer. On a d'ailleurs posé face à la montagne (Choëx).

Mais, en terme de sécurité, tout était tip, top. Aucun souci.
