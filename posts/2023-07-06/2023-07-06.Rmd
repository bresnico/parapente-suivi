...
# Sauvegardez la locale actuelle
...
d_post <- d |> 
  filter(Date == date_day) |> 
  select(date_vol,`Heure de décollage`, `Nomn de décollage`, `Nomn d'atterrissage`, `Durée`)

```

C'était le `r rmarkdown::metadata$date_vol`. Je suis parti de la région de `r d_post[3]` à `r d_post[2]` et j'ai atterri dans la région de `r d_post[4]` pour un durée de vol de `r d_post[5]`. J'ai volé `r rmarkdown::metadata$vols` fois dans la journée. J'ai ressenti `r rmarkdown::metadata$intense` de `r rmarkdown::metadata$emotion`.
